...
**Architecture:** Standalone Python-приложение, запускаемое вручную один раз (~6,5 часов). Перечень 700-ПП даёт состав (42 650 кадастровых номеров, из них 22 863 здания), геопортал НСПД даёт площадь по кадастровому номеру, адресный реестр Москвы даёт UNOM для вычитания точек. Обогащение идёт **после** фильтра по площади, поэтому дорогой шаг применяется к ~3 400 объектам, а не к 42 000. Каждое значение хранит провенанс; NULL легитимен.

> **Все числа ниже проверены разведкой на живых данных 2026-07-16**, а не взяты
> из головы. Блоки с пометкой «РАЗВЕДКА ПРОВЕДЕНА» содержат фактические результаты.
...
CREATE TABLE IF NOT EXISTS kosmos.objects (
    ...
    -- Обогащаем через НСПД только приложение 1: у помещений поле area —
    -- это площадь комнаты, а не здания. Экономит 19 480 запросов (~5,4 часа).
    annex               INTEGER,
...
> **РАЗВЕДКА ПРОВЕДЕНА 2026-07-16 НА ЖИВЫХ ДАННЫХ.** Проверено: PDF скачивается
> (8 381 824 байта, HTTP 200, `%PDF-1.7`), из него извлекается **42 650 уникальных
> кадастровых номеров** — приёмочный критерий выполнен. Разбивка по кадастровым
> округам: 77 (Москва) — 41 784, **50 (область) — 866**. Это Зеленоград и ТиНАО,
> исторически зарегистрированные в Московской области. **Не отбрасывайте их как
> «не Москву»** — это полноправные объекты перечня.
...
### ⚠️ Как устроен текст в этом PDF — читайте до того, как писать парсер

Автор плана провалился здесь дважды, прежде чем понял. **Наивный подход возвращает ноль молча, без единой ошибки** — вы решите, что PDF пустой.

Устройство:

1. **Subset-шрифты с собственной кодировкой глифов.** Кириллица (адреса) лежит не текстом, а номерами глифов: `[<00010002>-3<0003000400050003>]TJ`. Без таблицы ToUnicode это не декодировать.
2. **Цифры и латиница — открытым текстом** в литералах `(...)`. Кадастровые номера состоят из цифр и двоеточий, поэтому они извлекаемы.
3. **Кернинг режет числа на куски.** Это главная ловушка. Номер `50:09:0000000:179835` лежит в PDF как последовательность литералов `(50:09)` `(:0000)` `(000:1)` `(79835)` с кернинг-числами между ними. Регулярка по сырому потоку не найдёт ни одного номера.

Рабочий метод, проверенный на настоящем файле:

```
для каждого потока с операторами Tj/TJ:
    распаковать zlib
    вытащить ВСЕ литералы (...) регуляркой /\(((?:\\.|[^\\()])*)\)/g
    склеить их содержимое ПОДРЯД, без разделителей
    -> кернинг-числа и hex-глифы просто выпадают
затем регулярка \d{2}:\d{2}:\d{6,7}:\d+ по склеенному тексту
```

`pypdf` с извлечением текста должен справиться сам, потому что он обрабатывает
CMap и кернинг. **Но проверьте счётчик:** если получилось не 42 000–42 700, парсер
молча теряет данные. Считайте это приёмочным критерием, а не пожеланием.

**Адреса из PDF брать не нужно.** Они в глифах и потребуют CMap. Разведка показала,
что НСПД возвращает поле `readable_address` — канонический адрес прямо из ЕГРН,
чище чем в перечне. Берите адрес оттуда.
...
> **РАЗВЕДКА ПРОВЕДЕНА 2026-07-16.** Эндпоинт работает. Выборка 25 объектов из
> перечня: **площадь вернулась для 23 (92%)** — приёмочный критерий 60% перевыполнен.
> Все 25 — назначение «Нежилое»: жилого шума в перечне нет.
>
> **Численная проверка, что площадь настоящая, а не пересчёт геометрии.** Объект
> `77:01:0001075:1017`: площадь полигона из ответа — 588 м², `floors` = 12,
> `build_record_area` = 7198.6. Отношение **12,24 ≈ этажности**. Это учтённая
> площадь ЕГРН. Утверждение подтверждено независимым расчётом.
>
> **Бонус, которого не было в дизайне:** ответ содержит `readable_address`
> (канонический адрес ЕГРН), `cost_value` (кадастровая стоимость) и `materials`.
> Адрес берите отсюда, а не из PDF.
>
> **Категории в выборке:** Здания — 13, Помещения — 10, Сооружения — 1, не найден — 1.
> **«Сооружения» отдают `null` в площади** — код это обрабатывает, NULL легитимен.
...
class Registry:
    ...
    def upsert_from_pp700(self, entry: Pp700Entry) -> int:
        ...
        with self._conn.cursor() as cur:
            cur.execute(
                "INSERT INTO kosmos.objects (cadastral_no, address, annex, baseline_run) "
                "VALUES (%s, %s, %s, true) "
                "ON CONFLICT (cadastral_no) DO UPDATE "
                "SET address = EXCLUDED.address, annex = EXCLUDED.annex, "
                "    last_seen_at = now() "
                "RETURNING id",
                (cadastral_no, entry.address, entry.annex),
            )
            return cur.fetchone()[0]
...
**Контекст.** Порядок этапов определяет стоимость всего проекта. Обогащение идёт **после** фильтра: объекты через НСПД, фильтр 10 000 м² оставляет ~3 400, и только для них добираются название и трафик. Обратный порядок сделал бы проект неподъёмным.

Скрипт должен быть **возобновляемым**: часы работы нельзя терять из-за одного таймаута.

### ⚠️ Приложение 2 — ловушка на 5 часов. Нашла разведка

Перечень состоит из двух приложений, и они принципиально разные:

| Приложение | Объектов | Что это | Что отдаёт НСПД |
|---|---|---|---|
| 1 | 22 863 | **здания** | `build_record_area` — площадь **здания**. То, что нам нужно |
| 2 | 19 480 | **помещения** | `area` — площадь **комнаты**, не здания |

В выборке разведки все 10 помещений дали от 13,6 до 621,2 м². Это правильные цифры — но они про комнату.

**Почему это ловушка.** Фильтр 10 000 м² отсеет их корректно, так что база не испортится. Но мы потратим **19 480 запросов ≈ 5,4 часа** на объекты, которые заведомо не пройдут фильтр по своей природе.

**Почему их нельзя просто выбросить.** Сноска к приложению 1 объясняет: помещения указываются, когда **здание не стоит на кадастровом учёте**. То есть здание может быть настоящим БЦ, но его кадастрового номера у нас нет — есть только номера комнат в нём и адрес.

**Решение для этого плана:** обогащаем **только приложение 1**. Приложение 2 остаётся в реестре как есть, со статусом «не обогащено, здание не в кадастре», и обрабатывается отдельной веткой через поиск по адресу — **это отдельный план, не этот.** Так мы экономим 5,4 часа и не теряем данные.

Сохраняйте `annex` при загрузке — без него разделить не получится.
...
def test_pending_excludes_already_enriched(registry):
    """Возобновляемость: часы работы нельзя терять из-за таймаута."""
    registry.execute(
        "INSERT INTO kosmos.objects "
        "(cadastral_no, annex, area_sqm, area_source, area_confidence) "
        "VALUES ('77:01:0001:1', 1, 15000, 'nspd', 'точно')"
    )
    registry.execute(
        "INSERT INTO kosmos.objects (cadastral_no, annex) VALUES ('77:01:0001:2', 1)"
    )
...
def test_pending_skips_annex_2_rooms(registry):
    """Приложение 2 — помещения. НСПД отдаёт по ним площадь комнаты,
    а не здания: 19 480 запросов = 5,4 часа впустую. Их ветка — отдельный план.

    Разведка 2026-07-16: все 10 помещений выборки дали 13,6-621,2 м².
    """
    registry.execute(
        "INSERT INTO kosmos.objects (cadastral_no, annex) VALUES ('77:01:0001:1', 1)"
    )
    registry.execute(
        "INSERT INTO kosmos.objects (cadastral_no, annex) VALUES ('77:06:0003007:8714', 2)"
    )

    pending = pending_cadastrals(registry)

    assert pending == ["77:01:0001:1"]
```
...
def pending_cadastrals(registry: Registry) -> list[str]:
    """Кадастровые номера к обогащению. Основа возобновляемости.

    Только приложение 1 (здания). У помещений из приложения 2 поле area —
    это площадь комнаты, а не здания: они заведомо не пройдут фильтр 10 000 м²,
    и 19 480 запросов к ним — 5,4 часа впустую. Их ветка (поиск здания
    по адресу) — отдельный план.
    """
    with registry._conn.cursor() as cur:
        cur.execute(
            "SELECT cadastral_no FROM kosmos.objects "
            "WHERE area_source IS NULL AND annex = 1 ORDER BY id"
        )
        return [row[0] for row in cur.fetchall()]


def stage_enrich(registry: Registry, cfg: Config) -> None:
    """Обогатить площадью из ЕГРН. ~6,4 часа при 1 req/s. Возобновляемо."""
...
Expected: `от_10к` — **порядка 3 400**. Это и есть список лидов Димы.

Оценка выведена из разведки: в выборке 2 из 13 зданий (15%) оказались от 10 000 м²,
при 22 863 зданиях приложения 1 это ~3 400. Если получилось меньше 2 000 или больше
5 000 — разберитесь, прежде чем показывать Диме.
...
Этот план заканчивается работающим реестром в Postgres: 42 650 объектов, из них ~3 400 зданий от 10 000 м² с площадями из ЕГРН, с вычтенными точками МИЛТИ и полным журналом провенанса.

**Не входит в этот план и требует отдельных:**

1. **Ветка приложения 2** — 19 480 помещений в зданиях, не стоящих на кадастровом учёте. У нас есть их адрес, но не кадастровый номер здания. Нужен поиск здания по адресу через НСПД и склейка помещений в здание. Часть из них — настоящие БЦ, которых иначе в базе не будет.
2. **Витрина и еженедельный конвейер** — публикация в Google Sheets, конкуренты через 2ГИС, дельта, лист «В СТРОЙКЕ» из реестра разрешений на ввод, расписание в n8n.
3. **Telegram-бот и аналитик** — дайджест, диалог, вердикты кнопками.
