...
export class Registry {
  ...
  async pendingCadastrals() {
    ...
    // важно не «когда закончится», а «когда появится польза». Здания
    // с названием из OSM вдвое крупнее остальных (5438 м² против 2561)
    // и вчетверо чаще проходят порог 10 000 м² — 12,4% против 6,3%.
    // Значит их надо спрашивать первыми: за три часа они дают Диме
    // больше БЦ, чем случайный порядок за сутки.
    //
    // Внутри группы — по убыванию этажности, если она уже известна:
    // высокое здание почти всегда большое.
    const rows = await this.sql`
      SELECT cadastral_no FROM kosmos.objects
      WHERE area_source IS NULL AND annex = 1
      ORDER BY
        (title IS NULL),          -- сначала именованные
